...
function init() {
  return new Promise((resolve, reject) => {
    try {
      echo.log('(CERT) Root cert is loading...');
      ...
      echo.log('(CERT) Root cert is loaded successfully');
      resolve();
    } catch (err) {
      ...
      utils.mkdir(path.dirname(rootCertPath))
        ...
        .then(() => {
          echo.log('(CERT) Root cert is created and loaded successfully');
          resolve();
        })
        .catch(reject);
    }
...
function getCertInfo(domain) {
  echo.log(`(CERT) Create certificate for < ${domain} > successfully.`);
